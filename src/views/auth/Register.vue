  ...
  <div class="w-full max-w-md mx-auto">
    <div class="text-center mb-8">
      ...
      <p class="text-gray-600 dark:text-gray-400">
        ...
        <router-link to="/auth/login" class="font-semibold text-primary-600 hover:text-primary-500 transition-colors">
          Sign in
        </router-link>
      </p>
    </div>
    ...
    <form class="space-y-6" @submit.prevent="handleSubmit">
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            :class="{ 'border-red-500 focus:ring-red-500 focus:border-red-500': displayNameError }"
            placeholder="Enter your display name"
            @blur="displayNameError"
          />
          <p v-if="displayNameError" class="mt-1 text-sm text-red-500">{{ displayNameError }}</p>
        </div>
      ...
      <button
        ...
        :disabled="loading"
        class="w-full py-3 px-4 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 font-semibold transition-all duration-200 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700"
      >
...
const displayNameError = computed(() => {
  ...
  if (displayName.value.length < 3) return 'Display name must be at least 3 characters long';
  return '';
});
...
const isFormValid = computed(() => {
  return !emailError.value && !passwordError.value && !confirmPasswordError.value && !displayNameError.value && 
         email.value && password.value && confirmPassword.value && displayName.value;
});
...
const handleSubmit = async () => {
  try {
    ...
    // Register the user
    const user = await authStore.register(email.value, password.value, displayName.value);
    console.log(user);
